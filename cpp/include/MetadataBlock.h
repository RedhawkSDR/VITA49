...
namespace vrt {
  ...
  class MetadataEntry : public VRTObject {
    ...
    public: inline void setLevel (int32_t l) {
      if ((l < 0) || (l > 4095)) {
        throw VRTException("Invalid level (%d), value must be in range [0,4095]", l);
      }
      this->level = l;
    }
    ...
    /** Sets the key name. */
    public: inline void setKey (const string &k) {
      setKey(&k);
    }
    ...
    /** Sets the value. */
    public: inline void setValue (const wstring &v) {
      setValue(&v);
    }
  ...
  class MetadataBlock : public VRTObject {
    ...
    public: inline void setEntries (const vector<MetadataEntry> &_entries) {
